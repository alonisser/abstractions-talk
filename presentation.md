...
BUT, for SQL It (mostly) works.

---
# Hubris

And then some smart programmer thought it was a good idea to Abstract **ALL KINDS OF DATABASES**. Sql, Mongo, Redis, Disk, Memory. Everything goes

.img-half-container.center[![daedalus](daedalus.jpg)]

---

# Hubris

Unfortunately for us, by problematic early tech decisions, we were heavily invested in the web framework written by this programmer.
 
--

And yet again, our app was slow, cpu saturated, event loop blocked. 
...
```javascript
DataSource.find({}).populate('client').populate('parentDataSource)
```

In the land of SQL ORM we can probably all understand the meaning here:

>Get all of DataSource model instances, add join with foreign key fields *parentDataSource* and *client*
...
Since we had 1830 DataSource instances at the time, I now had a good idea what the Db Abstracted *ORM* was trying to do on **Join**

--

**Go to DB again and again for EVERY joined object**

---
# Joined and owned

What happens when we have a 3600 db queries waiting for one endpoint?

And the connection pool seemed to have about 50 connections. 

That's 3600/50 request cycles, multiple by 10ms for query and latency.

--

And in the meanwhile Nothing else can hit the db.

---

# Joined and owned

Once we wrote our own "join" logic, things looked quite different

.img-container[![cpu](cpu-ds.png)]
.img-container[![response time](datasource_controller.png)]

This "ORM" had one abstraction too many.
